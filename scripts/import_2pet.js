...
async function import2Peter() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['2Pet']);
        ...
        for (let chapterNum = 1; chapterNum <= 3; chapterNum++) {
            const apiUrl = `https://bible-api.com/2+peter+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 2Pet chapter ${chapterNum}`);
            ...
            console.log(`Imported 2Pet chapter ${chapterNum}`);
        }
        console.log('Import of 2Pet complete!');
    } catch (err) {
        console.error('Error importing 2Pet:', err);
    } finally {
        await pool.end();
    }
}
