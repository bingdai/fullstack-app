...
async function importEphesians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Eph']);
        ...
        for (let chapterNum = 1; chapterNum <= 6; chapterNum++) {
            const apiUrl = `https://bible-api.com/ephesians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Eph chapter ${chapterNum}`);
            ...
            console.log(`Imported Eph chapter ${chapterNum}`);
        }
        console.log('Import of Eph complete!');
    } catch (err) {
        console.error('Error importing Eph:', err);
    } finally {
        await pool.end();
    }
}
