...
async function importNumbers() {
    try {
        ...
        for (let chapterNum = 1; chapterNum <= 36; chapterNum++) {
            console.log(`Importing Numbers chapter ${chapterNum}...`);
            // Fetch Numbers chapter from Bible API
            const response = await axios.get(`https://bible-api.com/numbers+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Numbers chapterNum
        ...
        console.error('Error importing Numbers:', err);
    } finally {
        await pool.end();
    }
}
