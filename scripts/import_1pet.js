...
async function import1Peter() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['1Pet']);
        ...
        for (let chapterNum = 1; chapterNum <= 5; chapterNum++) {
            const apiUrl = `https://bible-api.com/1+peter+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 1Pet chapter ${chapterNum}`);
            ...
            console.log(`Imported 1Pet chapter ${chapterNum}`);
        }
        console.log('Import of 1Pet complete!');
    } catch (err) {
        console.error('Error importing 1Pet:', err);
    } finally {
        await pool.end();
    }
}
