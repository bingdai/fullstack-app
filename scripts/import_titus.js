...
async function importTitus() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Titus']);
        ...
        for (let chapterNum = 1; chapterNum <= 3; chapterNum++) {
            const apiUrl = `https://bible-api.com/titus+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Titus chapter ${chapterNum}`);
            ...
            console.log(`Imported Titus chapter ${chapterNum}`);
        }
        console.log('Import of Titus complete!');
    } catch (err) {
        console.error('Error importing Titus:', err);
    } finally {
        await pool.end();
    }
}
