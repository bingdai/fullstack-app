...
async function importMark() {
    try {
        // Get the book ID for Mark
        ...
        for (let chapterNum = 1; chapterNum <= 16; chapterNum++) {
            console.log(`Importing Mark chapter ${chapterNum}...`);
            // Fetch Mark chapter from Bible API
            const response = await axios.get(`https://bible-api.com/mark+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Mark chapterNum
        ...
        console.error('Error importing Mark:', err);
    } finally {
        await pool.end();
    }
}
