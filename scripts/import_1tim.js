...
async function import1Timothy() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['1Tim']);
        ...
        for (let chapterNum = 1; chapterNum <= 6; chapterNum++) {
            const apiUrl = `https://bible-api.com/1+timothy+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 1Tim chapter ${chapterNum}`);
            ...
            console.log(`Imported 1Tim chapter ${chapterNum}`);
        }
        console.log('Import of 1Tim complete!');
    } catch (err) {
        console.error('Error importing 1Tim:', err);
    } finally {
        await pool.end();
    }
}
