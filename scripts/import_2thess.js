...
async function import2Thessalonians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['2Thess']);
        ...
        for (let chapterNum = 1; chapterNum <= 3; chapterNum++) {
            const apiUrl = `https://bible-api.com/2+thessalonians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 2Thess chapter ${chapterNum}`);
            ...
            console.log(`Imported 2Thess chapter ${chapterNum}`);
        }
        console.log('Import of 2Thess complete!');
    } catch (err) {
        console.error('Error importing 2Thess:', err);
    } finally {
        await pool.end();
    }
}

import2Thessalonians();
