...
async function importGalatians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Gal']);
        ...
        for (let chapterNum = 1; chapterNum <= 6; chapterNum++) {
            const apiUrl = `https://bible-api.com/galatians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Gal chapter ${chapterNum}`);
            ...
            console.log(`Imported Gal chapter ${chapterNum}`);
        }
        console.log('Import of Gal complete!');
    } catch (err) {
        console.error('Error importing Gal:', err);
    } finally {
        await pool.end();
    }
}
