...
async function importRevelation() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Rev']);
        ...
        for (let chapterNum = 1; chapterNum <= 22; chapterNum++) {
            const apiUrl = `https://bible-api.com/revelation+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Rev chapter ${chapterNum}`);
            ...
            console.log(`Imported Rev chapter ${chapterNum}`);
        }
        console.log('Import of Rev complete!');
    } catch (err) {
        console.error('Error importing Rev:', err);
    } finally {
        await pool.end();
    }
}
