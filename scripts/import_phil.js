...
async function importPhilippians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Phil']);
        ...
        for (let chapterNum = 1; chapterNum <= 4; chapterNum++) {
            const apiUrl = `https://bible-api.com/philippians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Phil chapter ${chapterNum}`);
            ...
            console.log(`Imported Phil chapter ${chapterNum}`);
        }
        console.log('Import of Phil complete!');
    } catch (err) {
        console.error('Error importing Phil:', err);
    } finally {
        await pool.end();
    }
}
