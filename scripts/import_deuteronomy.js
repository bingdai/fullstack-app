...
async function importDeuteronomy() {
    try {
        // Get the book ID for Deuteronomy
        ...
        // Deuteronomy has 34 chapters
        for (let chapterNum = 1; chapterNum <= 34; chapterNum++) {
            console.log(`Importing Deuteronomy chapter ${chapterNum}...`);
            // Fetch Deuteronomy chapter from Bible API
            const response = await axios.get(`https://bible-api.com/deuteronomy+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Deuteronomy chapterNum
        ...
        console.error('Error importing Deuteronomy:', err);
    } finally {
        await pool.end();
    }
}
