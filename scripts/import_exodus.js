...
async function importExodus() {
    try {
        // Get the book ID for Exodus
        ...
        for (let chapterNum = 1; chapterNum <= 40; chapterNum++) {
            console.log(`Importing Exodus chapter ${chapterNum}...`);
            // Fetch Exodus chapter from Bible API
            const response = await axios.get(`https://bible-api.com/exodus+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Exodus chapterNum
        ...
        console.error('Error importing Exodus:', err);
    } finally {
        await pool.end();
    }
}
