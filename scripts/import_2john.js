...
async function import2John() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['2John']);
        ...
        for (let chapterNum = 1; chapterNum <= 1; chapterNum++) {
            ...
            const apiUrl = `https://bible-api.com/2+john+1-13`;
            ...
            for (const verse of verses) {
                const verseNumber = verse.verse; // Correct: API gives actual verse number (1-13)
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 2John chapter ${chapterNum}`);
            ...
            console.log(`Imported 2John chapter ${chapterNum}`);
        }
        console.log('Import of 2John complete!');
    } catch (err) {
        console.error('Error importing 2John:', err);
    } finally {
        await pool.end();
    }
}
