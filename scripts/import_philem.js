...
async function importPhilemon() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Phlm']);
        ...
        for (let chapterNum = 1; chapterNum <= 1; chapterNum++) {
            const apiUrl = `https://bible-api.com/philemon+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Philem chapter ${chapterNum}`);
            ...
            console.log(`Imported Philem chapter ${chapterNum}`);
        }
        console.log('Import of Philem complete!');
    } catch (err) {
        console.error('Error importing Philem:', err);
    } finally {
        await pool.end();
    }
}
