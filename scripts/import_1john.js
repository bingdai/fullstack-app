...
async function import1John() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['1John']);
        ...
        for (let chapterNum = 1; chapterNum <= 5; chapterNum++) {
            const apiUrl = `https://bible-api.com/1+john+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 1John chapter ${chapterNum}`);
            ...
            console.log(`Imported 1John chapter ${chapterNum}`);
        }
        console.log('Import of 1John complete!');
    } catch (err) {
        console.error('Error importing 1John:', err);
    } finally {
        await pool.end();
    }
}
