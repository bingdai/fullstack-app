...
async function importHebrews() {
    try {
        ...
        // Hebrews has 13 chapters
        for (let chapterNum = 1; chapterNum <= 13; chapterNum++) {
            console.log(`Importing Hebrews chapter ${chapterNum}...`);
            // Fetch Hebrews chapter from Bible API
            const response = await axios.get(`https://bible-api.com/hebrews+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Hebrews chapterNum
        ...
        console.error('Error importing Hebrews:', err);
    } finally {
        await pool.end();
    }
}
