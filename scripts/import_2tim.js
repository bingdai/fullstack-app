...
async function import2Timothy() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['2Tim']);
        ...
        for (let chapterNum = 1; chapterNum <= 4; chapterNum++) {
            const apiUrl = `https://bible-api.com/2+timothy+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 2Tim chapter ${chapterNum}`);
            ...
            console.log(`Imported 2Tim chapter ${chapterNum}`);
        }
        console.log('Import of 2Tim complete!');
    } catch (err) {
        console.error('Error importing 2Tim:', err);
    } finally {
        await pool.end();
    }
}
