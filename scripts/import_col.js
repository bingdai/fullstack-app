...
async function importColossians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Col']);
        ...
        for (let chapterNum = 1; chapterNum <= 4; chapterNum++) {
            const apiUrl = `https://bible-api.com/colossians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Col chapter ${chapterNum}`);
            ...
            console.log(`Imported Col chapter ${chapterNum}`);
        }
        console.log('Import of Col complete!');
    } catch (err) {
        console.error('Error importing Col:', err);
    } finally {
        await pool.end();
    }
}
