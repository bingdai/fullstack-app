...
const books = [
    { name: 'Joshua', short: 'Josh', chapters: 24 },
    { name: 'Judges', short: 'Judg', chapters: 21 },
    { name: 'Ruth', short: 'Ruth', chapters: 4 }
];
...
async function importBook(book) {
    ...
    for (let chapterNum = 1; chapterNum <= book.chapters; chapterNum++) {
        ...
        const response = await axios.get(`https://bible-api.com/${book.name.toLowerCase()}+${chapterNum}`);
