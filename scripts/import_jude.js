...
async function importJude() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Jude']);
        ...
        for (let chapterNum = 1; chapterNum <= 1; chapterNum++) {
            ...
            const apiUrl = `https://bible-api.com/jude+1-25`;
            ...
            for (const verse of verses) {
                const verseNumber = verse.verse; // Correct: API gives actual verse number (1-25)
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Jude chapter ${chapterNum}`);
            ...
            console.log(`Imported Jude chapter ${chapterNum}`);
        }
        console.log('Import of Jude complete!');
    } catch (err) {
        console.error('Error importing Jude:', err);
    } finally {
        await pool.end();
    }
}
