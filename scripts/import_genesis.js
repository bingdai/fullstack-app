...
async function importGenesis() {
    try {
        ...
        for (let chapterNum = 1; chapterNum <= 50; chapterNum++) {
            console.log(`Importing Genesis chapter ${chapterNum}...`);
            // Fetch Genesis chapter from Bible API
            const response = await axios.get(`https://bible-api.com/genesis+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Genesis chapterNum
        ...
        console.error('Error importing Genesis:', err);
    } finally {
        await pool.end();
    }
}
