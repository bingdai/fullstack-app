...
async function importJob() {
    try {
        ...
        // Job has 42 chapters
        for (let chapterNum = 1; chapterNum <= 42; chapterNum++) {
            console.log(`Importing Job chapter ${chapterNum}...`);
            // Fetch Job chapter from Bible API
            const response = await axios.get(`https://bible-api.com/job+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Job chapterNum
