...
async function importMatthew() {
    try {
        // Get the book ID for Matthew
        ...
        for (let chapterNum = 1; chapterNum <= 28; chapterNum++) {
            console.log(`Importing Matthew chapter ${chapterNum}...`);
            // Fetch Matthew chapter from Bible API
            const response = await axios.get(`https://bible-api.com/matthew+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Matthew chapterNum
        ...
        console.error('Error importing Matthew:', err);
    } finally {
        await pool.end();
    }
}
