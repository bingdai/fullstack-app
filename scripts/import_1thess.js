...
async function import1Thessalonians() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['1Thess']);
        ...
        for (let chapterNum = 1; chapterNum <= 5; chapterNum++) {
            const apiUrl = `https://bible-api.com/1+thessalonians+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in 1Thess chapter ${chapterNum}`);
            ...
            console.log(`Imported 1Thess chapter ${chapterNum}`);
        }
        console.log('Import of 1Thess complete!');
    } catch (err) {
        console.error('Error importing 1Thess:', err);
    } finally {
        await pool.end();
    }
}

import1Thessalonians();
