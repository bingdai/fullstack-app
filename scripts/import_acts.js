...
async function importActs() {
    try {
        ...
        for (let chapterNum = 1; chapterNum <= 28; chapterNum++) {
            console.log(`Importing Acts chapter ${chapterNum}...`);
            // Fetch chapter from Bible API
            const response = await axios.get(`https://bible-api.com/acts+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Acts chapterNum
        ...
        console.error('Error importing Acts:', err);
    } finally {
        await pool.end();
    }
}
