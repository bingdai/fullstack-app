...
async function importHebrews() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Heb']);
        ...
        for (let chapterNum = 1; chapterNum <= 13; chapterNum++) {
            const apiUrl = `https://bible-api.com/hebrews+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Heb chapter ${chapterNum}`);
            ...
            console.log(`Imported Heb chapter ${chapterNum}`);
        }
        console.log('Import of Heb complete!');
    } catch (err) {
        console.error('Error importing Heb:', err);
    } finally {
        await pool.end();
    }
}
