...
async function importJames() {
    try {
        const bookResult = await pool.query('SELECT id FROM bible.books WHERE short = $1', ['Jas']);
        ...
        for (let chapterNum = 1; chapterNum <= 5; chapterNum++) {
            const apiUrl = `https://bible-api.com/james+${chapterNum}`;
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) throw new Error(`Verse ${verseNumber} not found in Jas chapter ${chapterNum}`);
            ...
            console.log(`Imported Jas chapter ${chapterNum}`);
        }
        console.log('Import of Jas complete!');
    } catch (err) {
        console.error('Error importing Jas:', err);
    } finally {
        await pool.end();
    }
}
