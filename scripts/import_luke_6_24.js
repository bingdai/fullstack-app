...
async function importLukeChapters6to24() {
    try {
        // Get the book ID for Luke
        ...
        for (let chapterNum = 6; chapterNum <= 24; chapterNum++) {
            console.log(`Importing Luke chapter ${chapterNum}...`);
            // Fetch Luke chapter from Bible API
            const response = await axios.get(`https://bible-api.com/luke+${chapterNum}`);
            const verses = response.data.verses;

            // Get the chapter ID for Luke chapterNum
        ...
        console.error('Error importing Luke 6-24:', err);
    } finally {
        await pool.end();
    }
}

importLukeChapters6to24();
