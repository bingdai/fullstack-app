...
async function importJohn() {
    try {
        // Get the book ID for John
        ...
        for (let chapterNum = 1; chapterNum <= 21; chapterNum++) {
            // Fetch verses from Bible API (adjust URL as needed)
            const apiUrl = `https://bible-api.com/john+${chapterNum}`;
            const response = await axios.get(apiUrl);
            const verses = response.data.verses;

            // Get the chapter ID for John chapterNum
            ...
            for (const verse of verses) {
                ...
                if (verseResult.rows.length === 0) {
                    throw new Error(`Verse ${verseNumber} not found in John chapter ${chapterNum}`);
                }
            ...
            console.log(`Imported John chapter ${chapterNum}`);
        }
        console.log('Import of John complete!');
    } catch (err) {
        console.error('Error importing John:', err);
    } finally {
        await pool.end();
    }
}
